function P = JointDegreeDist6(K,alpha,beta,theta,delta,phi,gamma,a,b,c,d,f,g)
...
J = 0;
for node=1:T
    if Ba(node,alpha) && Bb(node,beta) && Bc(node,theta) && Bd(node,delta) && Bf(node,phi) && Bg(node,gamma)
        J=J+1;
    end
end
P=J/T;
